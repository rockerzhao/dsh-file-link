		...
		*/
		...
		function tryJumpNow() {
			...
			for (const el of candidates) {
				...
				if (lineEl !== null) scheduleCenterVerification(lineEl);
				else pendingPaint = {
					view,
					contentEl: el,
					startLine,
					endLine: lastLine,
					deadline: Date.now() + 3e3
				};
				pending = null;
				return true;
			}
		...
		function scheduleJump(bs, absolute, line, endLine) {
			...
			const tick = () => {
				...
				if (pending !== null) {
					if (!alive) pending = null;
					else if (tryJumpNow()) pending = null;
				}
				if (pendingPaint !== null) {
					if (!alive || retryPendingPaint()) pendingPaint = null;
				}
				if (pending !== null || pendingPaint !== null) window.setTimeout(tick, 100);
			};
